...
/*++
  ...
  Returns: none
--*/
void CEDKIIBuildDataViewerDlg::OnBnClickedSelectYLog()
{
	CFileDialog		fd(TRUE, L"*.log", NULL, OFN_DONTADDTORECENT | OFN_ENABLESIZING, _T("Log files (*.log)|*.log|All files (*.*)|*.*||"));

	fd.m_ofn.lpstrTitle = _T("Select EDKII BIOS build -y report file");
	if (fd.DoModal() == IDOK) {
		CStdioFile		csf;
		...
		if (csf.Open(fd.GetPathName(), CFile::modeRead | CFile::typeText)) {
			...
			while (!bValidated) {
				...
				if (lineNum == 1) {
					if (fileStr.Find(_T("Platform Summary")) == 0)
						...
						MessageBox(_T("Invalid -y report file!"), _T("ERROR"),  MB_ICONERROR);
						break;
					}
				}
			...
			m_pwndProgress = new CXProgressWnd(this, _T("Parsing build -y report file"), FALSE, FALSE);
			...
			while (bValidated && !bHeaderFound) {
				...
				if (fileStr.Find(_T("Report Content:")) == 0) {
					...
					// clear controls before parsing -Y file
					//
					...
					// clear internal data structures before parsing -Y file
					...
					while ((resToken = fileStr.Tokenize(_T(", "), curPos)) != _T("")) {
						if (resToken.Compare(_T("PCD")) == 0) m_ylogPcd.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("LIBRARY")) == 0) m_ylogLibrary.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("FLASH")) == 0) m_ylogFlash.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("DEPEX")) == 0) m_ylogDepex.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("BUILD_FLAGS")) == 0) m_ylogBuildFlags.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("FIXED_ADDRESS")) == 0) m_ylogFixedAddress.SetCheck(BST_CHECKED);
						else if (resToken.Compare(_T("EXECUTION_ORDER")) == 0) m_ylogExecutionOrder.SetCheck(BST_CHECKED);
					}
			...
			m_buildYLog = fd.GetPathName();
			// Replace backslash with slash so code that searches paths can expect one format to find.
			m_buildYLog.Replace(_T('\\'), _T('/'));
			GetDlgItem(IDC_EDIT_Y_LOG)->SetWindowTextW(m_buildYLog);
			m_filePcd = m_buildYLog + _T("\r\nPCD section");
			m_fileFlash = m_buildYLog + _T("\r\nFLASH section");
			m_fileModuleSummary = m_buildYLog + _T("\r\nModule Summary sections");
...
//									m_cvTreeModuleSummary.GetTreeCtrl().SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);
									...
									if (moduleCounter == 0)
										m_fileGuidModule = m_buildYLog + _T("\r\nModule Summary Execution Order");
